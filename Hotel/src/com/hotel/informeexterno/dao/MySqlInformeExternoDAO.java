	...
	public Object registrarIE(InformeExternoDTO ie,
		...
		try {  
			...
			for (InformeExternoDetalleDTO  detalleInformeExternoDTO : ieDet) {  
				...
				System.out.println("CodProductoProveedor:"+ocDet.getCod_articulo_proveedor());
				ArticuloProveedorDTO prodProvee =  (ArticuloProveedorDTO) sesion.selectOne("productoproveedor.SQL_getProductoProveedorXCodProductoProveedor",ocDet.getCod_articulo_proveedor());
				System.out.println("codProducto:"+prodProvee.getCod_articulo()); 
				ArticuloDTO  objProducto = (ArticuloDTO) sesion.selectOne("articulo.SQL_getProductoXCodProd",prodProvee.getCod_articulo()); 
				System.out.println("Stock del Producto:"+objProducto.getStock_articulo());
				System.out.println("Cantidad del Producto:"+ocDet.getCantidad());
				Integer stock=0; 
				if(objProducto.getStock_articulo()==null)
					objProducto.setStock_articulo(0); 
				if("Entrada".equals(ie.getTipo_informe_externo())){
					stock =  objProducto.getStock_articulo() +  ocDet.getCantidad();
				}else{
					stock =  objProducto.getStock_articulo() -  ocDet.getCantidad();
				} 
				objProducto.setStock_articulo(stock);
				if("Abastecimiento".equals(ordemperu.getTipo_ordencompra())){
